			...
			<div class="col-md-10 col-md-offset-1">
				<div class="product-detail-wrap">
					<div class="row">
						<?php
						$prd_id = $_GET['prd_id'];
						$sql_detail = "SELECT * FROM products WHERE prd_id = $prd_id ";
						$row = mysqli_fetch_array(mysqli_query($conn, $sql_detail));
						?>
						<div class="col-md-5">
						...
						<div class="col-md-7">
							...
							if (isset($_POST['sbm'])) {
								...
								header("location: modules/cart/cart_add.php?prd_id=".$prd_id."&qtt=".$qtt);
							}
							?>
							<form name="sbm" method="POST" enctype="multipart/form-data">

								<div class="desc">
									<h3> <?= $row['name'] . " " . $row['code'] ?></h3>
									<p class="price">
										<span><?= number_format($row['price']) ?> đ</span>
									</p>
									<p>thông tin</p>
									<p style="text-align: justify;">
										VIETPRO STORE sẽ giao hàng tận nơi khi chọn mua sản phẩm: <?= $row['name'] . " " . $row['code'] ?>. Hoặc quí khách có thể đến tại địa chỉ shop có hiển thị bên dưới, khi chọn size phù hợp để xem và thử trực tiếp.
									...
									<div class="row row-pb-sm">
										<div class="col-md-4">
											<div class="input-group">
												...
												<input type="text" id="quantity" name="qtt" class="form-control input-number" value="1" min="1" max="100">
												<span class="input-group-btn">
									...
									<p><button class="btn btn-primary" name="sbm" type="submit"> Thêm vào giỏ hàng</button></p>
							</form>
						</div>
						</form>
					</div>
